...
:: от lelik_01
::
:: Minecraft можно взять отсюда: https://github.com/lelik01/mcBatchLauncher/releases

:: путь к Java (можно найти в Program Files)
set JAVA="C:\Program Files\Java\jre1.8.0_131\bin\javaw.exe"

:: укажите папку, где будет находиться Minecraft
set MINECRAFT_DIR=%APPDATA%\.minecraft

:: папка с Minecraft (можно не трогать)
set GAME_DIR=%MINECRAFT_DIR%

:: ресурсы игры (можно не трогать)
set ASSETS_DIR=%MINECRAFT_DIR%\assets

:: серия версии Minecraft (можно не трогать)
set VERSION_SERIES=1.12

:: версия Minecraft (можно не трогать)
set VERSION=1.12

:: дополнительные библиотеки Minecraft (можно не трогать)
...
:: аргументы, кол-во выделяемого ОЗУ обязательно! (в мегабайтах = -Xmx512M, в гигабайтах = -Xmx1G)
set ARGUMENTS="-Xmx1G"

:: запуск Minecraft (можно не трогать)
cd %MINECRAFT_DIR%
